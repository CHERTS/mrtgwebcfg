...
if (!(isset($id)) ) {
	...
	for ($i=1; $i<=$index_rows; $i++) {
		...
		if($SQL_Type == "mysql") {
			$result = mysql_query("select distinct agent.id, agent.title, mrtg.filename, templates.row_set, templates.column_set
							from agent, mrtg, mrtg_group, templates
							where mrtg_group.id=templates.group_id and templates.agent_id=agent.id and templates.agent_id=mrtg.id and templates.hide_set=1 and agent.trash=0 and mrtg_group.id=".$gid." and templates.row_set=".$i.$sql_errors."
		...
		if ( $rows == 1 ) {
			...
			$file = $row_global[0].$separator.$row[2]."-day.gif";
			...
			if ($row[4] == 1) {
				...
				else $href = "<br><font color='#FF0000'><b>$MRTGMsg[113]</b></font>";
				print "<tr bgcolor='#F0F0F0'><td width='50%'><table cellpadding=4 cellspacing=0 width='100%'><tr><td><b>$row[1]</b></td></tr><tr><td>$href</td></tr></table></td><td width='50%'></td></tr>\n";
			} else {
				if ( file_exists($file) ) $href = "<a href='$self?gid=$gid&amp;mode=view&amp;id=$row[0]&amp;page=$page'><img src='$MRTG_Stat_Patch$row[2]-day.gif' border=0 alt='$MRTGMsg[183] $fdate $MRTGMsg[184]'></a>";
				else $href = "<br><font color='#FF0000'><b>$MRTGMsg[113]</b></font>";
				print "<tr bgcolor='#F0F0F0'><td width='50%'></td><td width='50%'><table cellpadding=4 cellspacing=0 width='100%'><tr><td><b>$row[1]</b></td></tr><tr><td>$href</td></tr></table></td></tr>\n";
			}
			...
			$file_1 = $row_global[0].$separator.$row_1[2]."-day.gif";
			$file_2 = $row_global[0].$separator.$row_2[2]."-day.gif";
			...
			else $href_1 = "<br><font color='#FF0000'><b>$MRTGMsg[113]</b></font>";
			...
			else $href_2 = "<br><font color='#FF0000'><b>$MRTGMsg[113]</b></font>";

		        print "<tr bgcolor='#F0F0F0'><td width='50%'>
			<table cellpadding=1 cellspacing=0 width='100%'><tr><td><b>$row_1[1]</b></td></tr><tr><td>$href_1</td></tr></table>
			</td><td width='50%'>
			<table cellpadding=1 cellspacing=0 width='100%'><tr><td><b>$row_2[1]</b></td></tr><tr><td>$href_2</td></tr></table>
			</td></tr>\n";
		}
	...
	print "\n<br><table cellpadding=0 cellspacing=5><tr align=center>\n";
	if($page == "errors") print "<td><form method=post ACTION='$self?gid=$gid&amp;mode=view'><input type=submit class='submit_main_button' value='$MRTGMsg[24]'></form></td>\n";
	else print "<td><form method=post ACTION='$self?gid=$gid&amp;mode=view&amp;page=errors'><input type=submit class='submit_main_button' value='$MRTGViewMsg[2]'></form></td>\n";
	print "<td><form method=post ACTION='admin/index.php'><input type=submit class='submit_main_button' value='$MRTGViewMsg[1]'></form></td>\n";
	print "</tr></table></div>\n";

} else {

	if($SQL_Type == "mysql") {
		$result = mysql_query("select agent_ip.ip, agent_ip.title, agent.title, agent.ver_snmp, mrtg.maxbytes, mrtg.options, mrtg.sfilename
						from agent,agent_ip,mrtg
						where agent.id=mrtg.id and agent.ip=agent_ip.id and agent.id=".$id);
		$row = mysql_fetch_row($result);
	} else {
		$result = pg_query($db, "select agent_ip.ip, agent_ip.title, agent.title, agent.ver_snmp, mrtg.maxbytes, mrtg.options, mrtg.sfilename
						from agent,agent_ip,mrtg
						where agent.id=mrtg.id and agent.ip=agent_ip.id and agent.id=".$id);
		$row = pg_fetch_row($result);
	}
	...
	if($row[5] != "") {
		...
		for($ii=0; $ii<$cnt; $ii++) {
			...
			if(strtolower($col[$ii]) == "growright" && $growright_set == 0) $growright_set = 1;
		}
	}
	...
	if($growright_set == 0) {
		...
		if($row[3] == "0") $full_target = "OTHER: ".$row[6];
		else $full_target = "SNMP: ".$ip[0];
		print "<tr bgcolor='#F0F0F0'><td width=30%><b>&nbsp;Target</b></td><td>&nbsp;".$full_target."</td></tr>";
		print "<tr bgcolor='#F0F0F0'><td width=30%><b>&nbsp;$MRTGMsg[2]</b></td><td>&nbsp;".$row[2]."</td></tr>";
		if($row[4]) {
			if ($row[4] < 1024) $row[4] = $row[4]." Bytes/s";
			elseif ($row[4] >= 1024 && $row[4] < 1048576) $row[4] = number_format($row[4]/1024, 2)." KBytes/s";
			else $row[4] = number_format($row[4]/1048576, 2)." MBytes/s";
			print "<tr bgcolor='#F0F0F0'><td width=30%><b>&nbsp;$MRTGMsg[149]</b></td><td>&nbsp;".$row[4]."</td></tr>";
		}
	}
	print "</table></div><br>";
	...
	print "<td><form method=post ACTION='$self?gid=$gid&amp;mode=view'><input type=submit class='submit_main_button' value='$MRTGMsg[24]'></form></td>\n";
	print "<td><form method=post ACTION='admin/index.php'><input type=submit class='submit_main_button' value='$MRTGViewMsg[1]'></form></td>\n";
	print "</tr></table></div>\n";
