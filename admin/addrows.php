...
if ( $gid != '') {
	$Set_Errors = 0;
	if($SQL_Type == "mysql") {
		if( $Mode_Add_Rows == "0" ) $result = mysql_query("select templates.id, templates.agent_id, templates.row_set, templates.column_set from mrtg_group,templates where mrtg_group.id=templates.group_id and templates.group_id = $gid and mrtg_group.id != $GID_Trash and templates.row_set != 0 and templates.row_set >= $addrows order by templates.row_set,templates.column_set asc");
